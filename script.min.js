...
if (moreBtn) {
  ...
  // 👉 Outside click to hide cards one by one
  document.addEventListener("click", e => {
    if (
      servicesVisible &&
      !moreBtn.contains(e.target) &&
      ![...extraCards].some(card => card.contains(e.target))
    ) {
      extraCards.forEach((card, i) => {
        setTimeout(() => {
          card.classList.add("opacity-0", "translate-y-8"); // fade-out + slide-down
          setTimeout(() => {
            card.classList.add("hidden");
          }, 400);
        }, i * 300); // delay ek-ek karke hide hone ke liye
      });

      servicesVisible = false;
      moreBtn.classList.remove("pointer-events-none", "opacity-70");
    }
  });
}
